...
const desktopNav = () => {
  ...
  let lastItemPrice = cart.length > 0 ? cart[cart.length - 1].price : 0;
  desktopButtonsNav.innerHTML = `
  <button id="toggle-mode">DARK</button>
  <span>Last Item <span class="last-item"> € ${lastItemPrice} </span></span>
...
card.querySelector(".add-to-cart").addEventListener("click", () => {
  const isDishInCart = cart.some((cartDish) => cartDish.id === dish.id)
  if (!isDishInCart) {
      dish.quantity = 1; // set initial quantity to 1
      cart.push(dish)
      updateCart()
  }
})
...
function updateCart() {
  ...
  cartElement.innerHTML = `
      <div class="price-expanded">
          ...
          ${cart.map((dish, index) => `
              <li>
                  <p>${dish.name} (x${dish.quantity}): <span class="price-item"> $${dish.price}</span></p>
          ...
          <hr />
          <p>Total: $${cart.reduce((total, dish) => total + (dish.price * dish.quantity), 0).toFixed(2)}</p>
  ...
  card.querySelector(".add-to-cart").addEventListener("click", () => {
    ...
    if (!isDishInCart) {
        ...
        const index = cart.findIndex((cartDish) => cartDish.id === dish.id)
        cart[index].quantity++
        updateCart()
    }
})
...
desktopNav() // update the navbar after updating the cart
}
